		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkExists("sase_network.n6", &network),
...
data "sase_networks" "all5" {
	depends_on = [
    	sase_network.n6
  	]
}

resource "sase_gateway"  "g1"{
  ...
  network_id = sase_network.n6.id
  region_id = {
    for network in data.sase_networks.all5.networks :
    network.id => network.regions[0].id
    if network.id == sase_network.n6.id
  }[sase_network.n6.id]
}
  `
